...
2. 单目位运算符，可以对操作数的每一位汇总运算，如`reg[31:0] B; wire A; assign A = &B;`
...
+ 非阻塞赋值 `<=` 并发执行（在上一阻塞执行之后）
  + **每一条非阻塞赋值执行前**，右面参与运算的变量值被保存在 **快照**，中，用快照计算
...
module test_bench;
    ...
    //实例化 Instantiate the Unit Under Test (UUT)
    Unit unit (
        .a(a_input),
        .b(b_putput)
    );
