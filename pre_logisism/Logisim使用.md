...
![image-20251013143849865](./assets/image-20251013143849865.png)

+ 灰色点处为**选项** 

  带灰色小圆点的那个端口是**选择信号输入端口**，待选择信号从灰色的 0 开始依次向下或者向上递增，如果是 2 位选择信号那从上到下就是 00,01,10,11

+ 灰色点右侧为使能信号

## DMX 多路分配器

![image-20251013144214437](./assets/image-20251013144214437.png)

+ 顾名思义，将某一个数据分**配到某个输出端口**中，与MUX恰好相反，例如应用在**计算机内存写入**中
+ 灰色点处为**选项** 
...
+ 扫描多位数输入，找到符合 “Type 属性” 定义的位，并**输出该位的 “索引”**（***从最低位 0 开始计数***）。
...
Type 属性决定了 “查找哪种位”，核心是 “扫描方向”（从低到高 / 从高到低）和 “目标位值”（0/1）的组合：
...
- **东边缘输出（索引输出）**：输出目标位的索引值（如 2、7）；，输出 0。
...
## Comparator 比较器 较为简单 略

![image-20251013143748318](./assets/image-20251013143748318.png)
...
| 引脚类型                     | 位置               | 功能描述                                                     |
| ---------------------------- | ------------------ | ------------------------------------------------------------ |
| **Data Input（数据输入）**   | 西边缘（元件左侧） | 输入需要拆分的多位数数据（如 8 位的`01010101`）。            |
| **Select Input（选择输入）** | 南边缘（元件下方） | 输入 “位组编号”，**指定要输出的位组**（组号需在有效范围内，**否则输出全 0**）。 |
| **Group Output（位组输出）** | 东边缘（元件右侧） | **超过最高位的都输出0**                                      |
...
1. http://flyinglandlord.github.io/2021/10/13/BUAA-CO-2021/Pre/Logisim复习/
2. logisim 官方文档