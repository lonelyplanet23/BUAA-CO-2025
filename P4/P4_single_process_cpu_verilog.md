...
4. Controller: 注意 always块内要赋初值，防止latch
5. NPC PC 一定都要初始化！
6. 写入是时序逻辑，而读出是组合逻辑！
7. **reg类型只能在always模块中赋值，wire 类型只能用assign赋值**
8. 明天注意：如果前程序有错误，尝试在npc多加一个端口imm26,接入立即数26；或者修改assign A3语句的选择顺序；或者将RA连接到GRF的RD1中，通过前面A3的选择读出`$31`的地址而非直接连接到.
   ![图1](assets/image.png)
## 模块说明
...
| RA[31:0] | 32 | O | $ra内存储的地址 |
| A3[4:0] | 5 | I | 写入地址 |
| WD[31:0] | 32 | I | 写入数据 |
| WE | 1 | I | 写入使能信号 |
| Clk | 1 | I | 时钟信号 |
| Reset | 1 | I | 同步复位 |
| PC[31:0] | 32 | I | 调试：当前指令地址 |
...
# **MIPS 单周期 CPU 控制信号表（宏定义版）**

| 指令    | func    | op   | RegDst\<1:0\> | ALUSrc | RegSrc\<1:0\> | RegWrite | MemWrite | nPC\_sel\<1:0\> | ExtOp | ALUcTRL\<2:0\>       | 功能描述        |
| :------ | :------ | :--- | :------------ | :----- | :------------ | :------- | :------- | :-------------- | :---- | :------------------- | :-------------- |
| **add** | ADD     | ZERO | RD\_RD        | 0      | RS\_ALU       | 1        | 0        | NPC\_PC4        | X     | ALU\_ADD             | R型加法         |
| **sub** | SUB     | ZERO | RD\_RD        | 0      | RS\_ALU       | 1        | 0        | NPC\_PC4        | X     | ALU\_SUB             | R型减法         |
| **ori** | n/a     | ORI  | RD\_RT        | 1      | RS\_ALU       | 1        | 0        | NPC\_PC4        | 0     | ALU\_OR              | 立即数或        |
| **lw**  | n/a     | LW   | RD\_RT        | 1      | RS\_MEM       | 1        | 0        | NPC\_PC4        | 1     | ALU\_ADD             | 读内存（Load）  |
| **sw**  | n/a     | SW   | X             | 1      | X             | 0        | 1        | NPC\_PC4        | 1     | ALU\_ADD             | 写内存（Store） |
| **beq** | n/a     | BEQ  | X             | 0      | X             | 0        | 0        | NPC\_BRANCH     | 1     | ALU\_SUB             | 分支相等跳转    |
| **lui** | n/a     | LUI  | RD\_RT        | 1      | RS\_ALU       | 1        | 0        | NPC\_PC4        | 0     | ALU\_SHIFT\_LEFT\_16 | 加载高位立即数  |
| **nop** | 00 0000 | ZERO | X             | 0      | X             | 0        | 0        | NPC\_PC4        | X     | ALU\_ADD             | 无操作指令      |
| **jal** | n/a     | JAL  | RD\_RA        | 0      | RS\_PC4       | 1        | 0        | NPC\_JUMP       | X     | ALU\_ADD             | 跳转并链接      |
| **jr**  | JR      | ZERO | X             | 0      | X             | 0        | 0        | NPC\_JR         | X     | ALU\_ADD             | 寄存器跳转      |
